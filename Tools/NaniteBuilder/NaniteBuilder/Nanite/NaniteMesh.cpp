	...
	bool NaniteMesh::Build(const Mesh& originMesh, int leafTriThreshold)
	{
		...
		for (int depth = 0; depth < MAX_DEPTH - 1; ++depth)
		{
			...
			for (int ci = 0; ci < srcClusters.size(); ++ci)
			{
				...
				auto simp = qem::SimplifyMesh(slicedTriangles, srcMesh.Vertices, numVerticesInCluster / 4);
				...
				Cluster tmpCluster;
				tmpCluster.StartIndex = dstMesh.Triangles.size();
				tmpCluster.NumTriangles = simp.second.size();

				dstMesh.Vertices.insert(dstMesh.Vertices.end(), simp.first.begin(), simp.first.end());
				dstMesh.Triangles.insert(dstMesh.Triangles.end(), simp.second.begin(), simp.second.end());
				dstClusters.push_back(tmpCluster);
			}
